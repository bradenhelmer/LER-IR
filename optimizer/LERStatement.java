				...
				if (prevLoop != null) {
					...
					if (l.loopType == LoopType.SUMMATION || prevLoop.loopType == LoopType.SUMMATION) {
						...
						if (l.uBound.equals(prevLoop.id)) {
							...
							try {
								int prevUboundValue = Integer.parseInt(prevLoop.uBound);
								newLoop.uBound = Integer.toString(prevUboundValue * prevUboundValue);
							} catch (NumberFormatException e) {
								// Handle the case where uBound is not an integer-parsable string
								newLoop.uBound = prevLoop.uBound; // Or some other fallback logic
							}
